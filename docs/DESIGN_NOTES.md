...
| 前端互動 | Hotwire(Turbo + Stimulus) | **整套 SPA + JSON API** |
...
這類應用可以非常複雜(Sendora 本身就是:多租戶、權限關聯、批量寄送、即時儀表板)。
Rails 撐不住的從來不是「太複雜」,而是「**形狀不對**」(見下方〈甜蜜點的邊界〉)。
別把「適合 Rails」讀成「只能做簡單網頁」。

「**Hotwire 省掉前端工程師**」這句**對一半**。精確說,它省掉的是「**另一套前端 codebase,
以及維護它的人**」——讓你跳過那筆稅:建 JSON / GraphQL API + 一個把領域型別在 TS 再實作一遍的
client app + 兩邊永遠在對帳。
...
User / Team / Membership 的關聯、交易信 + 批量寄送、即時 SQL 儀表板。
整個需求**沒有一處想變成 SPA**。
